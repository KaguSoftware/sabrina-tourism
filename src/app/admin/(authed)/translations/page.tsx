...
export default async function TranslationsPage() {
  ...
  return (
    <div className="max-w-[1100px]">
      ...
      {result.error ? (
        <div className="border border-terracotta/40 bg-terracotta/10 px-5 py-4 mb-6">
          <p className="font-mono text-[11px] tracking-[0.18em] uppercase text-terracotta mb-1">Error</p>
          <p className="text-[13px] text-ink-soft">{result.error}</p>
        </div>
      ) : (
        <>
          ...
          <TranslationsEditor initialMessages={result.data ?? ({} as Parameters<typeof TranslationsEditor>[0]["initialMessages"])} />
        </>
      )}
    </div>
  );

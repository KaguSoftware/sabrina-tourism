...
import { GripVertical, Pencil, Copy, Trash2, Sun } from "lucide-react";
import { useTranslations } from "next-intl";
import { reorderDailyPackages, setDailyPublished, deleteDailyPackage, duplicateDailyPackage } from "./actions";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-sm" onClick={deleting ? undefined : onClose}>
      <div className="bg-cream border border-rule p-8 max-w-sm w-full mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <p className="font-mono text-[11px] tracking-[0.16em] uppercase text-ink font-semibold mb-3">{t("confirm.deleteDailyTitle")}</p>
        <p className="font-sans text-[14px] text-ink-soft leading-relaxed mb-8">
          {name ? (<>Delete <span className="text-ink font-semibold">&quot;{name}&quot;</span>? This permanently removes the tour and can&apos;t be undone.</>) : t("confirm.deleteDailyBody")}
        </p>
  ...
  return (
    <tr ref={setNodeRef} style={style} className="border-b border-rule hover:bg-cream-warm/40 transition-colors">
      ...
      <td className="px-3 py-3 hidden md:table-cell">
        <span className="font-mono text-[10px] text-ink-soft">{pkg.region}</span>
      </td>
      ...
      <td className="px-3 py-3 w-28">
        <div className="flex items-center gap-1">
          <Link href={`/admin/daily/${pkg.id}`} title={t("edit")} aria-label={t("edit")} className="inline-flex items-center gap-1 px-2.5 py-1.5 font-mono text-[10px] tracking-[0.14em] uppercase text-ink-soft border border-rule hover:border-ochre hover:text-ochre transition-colors"><Pencil size={11} /></Link>
...
export function DailyTable({ initialPackages }: { initialPackages: AdminDailyRow[] }) {
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    const result = await reorderDailyPackages(reordered.map((p) => p.id));
    if (result.error) { setPackages(packages); toastError("reorder daily tours", result.error); }
    else toastReordered("Daily tour");
  }, [packages]);
  ...
  const handleTogglePublished = useCallback(async (id: string, current: boolean) => {
    ...
    const result = await setDailyPublished(id, next);
    if (result.error) { setPackages(packages); toastError(next ? "publish tour" : "unpublish tour", result.error); }
    else if (next) toastPublished("daily tour", target?.name);
    else toastUnpublished("daily tour", target?.name);
  }, [packages]);
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    const target = packages.find((p) => p.id === id);
    setDeleting(true);
    const result = await deleteDailyPackage(id);
    setDeleting(false);
    setDeleteTarget(null);
    if (result.error) { toastError("delete daily tour", result.error); }
    else { setPackages((prev) => prev.filter((p) => p.id !== id)); toastDeleted("daily tour", target?.name); }
  }, [deleteTarget, packages]);

  const handleDuplicate = useCallback(async (id: string) => {
    const target = packages.find((p) => p.id === id);
    const result = await duplicateDailyPackage(id);
    if (result.error) toastError("duplicate daily tour", result.error);
    else { toastDuplicated("daily tour", target?.name); router.push(`/admin/daily/${result.newId}`); }
  }, [router, packages]);
  ...
  if (packages.length === 0) {
    return (
      <EmptyState
        icon={Sun}
        title="No daily tours yet"
        description="Add your first daily tour to start building the catalog."
        action={
          <Link href="/admin/daily/new" className="inline-flex items-center gap-2 px-4 py-2.5 font-mono text-[11px] tracking-[0.16em] uppercase font-medium bg-ochre text-navy border border-ochre hover:bg-gold hover:border-gold transition-all duration-200 active:opacity-80">{t("pages.daily.new")}</Link>
        }
      />
    );
  }
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-rule">
                ...
                <th className="px-3 py-2 text-left font-mono text-[10px] tracking-[0.14em] uppercase text-ink font-semibold hidden md:table-cell">{t("common.region")}</th>

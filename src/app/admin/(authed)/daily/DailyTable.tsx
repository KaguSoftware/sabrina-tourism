...
export function DailyTable({ initialPackages }: { initialPackages: AdminDailyRow[] }) {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    if (result.error) { setPackages(packages); toast.error(result.error); }
    else toast.success("Daily tour deleted.");
  ...
  if (packages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-6">
        ...
        <Link href="/admin/daily/new" className="inline-flex items-center gap-2 px-4 py-2.5 font-mono text-[11px] tracking-[0.16em] uppercase font-medium bg-ochre text-navy border border-ochre hover:bg-gold hover:border-gold transition-all duration-200 active:scale-[0.97]">{t("pages.daily.new")}</Link>
      </div>
    );
  }

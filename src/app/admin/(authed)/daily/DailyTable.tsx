  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-sm" onClick={deleting ? undefined : onClose}>
      <div className="bg-cream border border-rule p-8 max-w-sm w-full mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="font-sans text-[14px] text-ink-soft leading-relaxed mb-8">{t("confirm.deleteDailyBody")}</p>
    ...
    <tr ref={setNodeRef} style={style} className="border-b border-rule hover:bg-cream-warm/40 transition-colors">
      ...
      <td className="px-3 py-3 w-28">
        <div className="flex items-center gap-1">
          <Link href={`/admin/daily/${pkg.id}`} className="inline-flex items-center gap-1 px-2.5 py-1.5 font-mono text-[10px] tracking-[0.14em] uppercase text-ink-soft border border-rule hover:border-ochre hover:text-ochre transition-colors"><Pencil size={11} /></Link>
...
export function DailyTable({ initialPackages }: { initialPackages: AdminDailyRow[] }) {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    else { setPackages((prev) => prev.filter((p) => p.id !== id)); toast.success("Daily tour deleted."); }
  }, [deleteTarget]);

  const handleDuplicate = useCallback(async (id: string) => {
    const result = await duplicateDailyPackage(id);
    if (result.error) toast.error(result.error);
    else { toast.success("Duplicated. Edit the copy."); router.push(`/admin/daily/${result.newId}`); }
  }, [router]);

  if (packages.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-6">
        <p className="text-[24px] text-ink-soft italic" style={{ fontFamily: "var(--font-fraunces)" }}>{t("pages.daily.title")}</p>

...
export function MultiPersonPrices({ pricing, currency }: MultiPersonPricesProps) {
  ...
  const slots = [
    ...
    { key: "threePeople", label: t("threePeople"), price: pricing.threePeople, icon: <THREE_PEOPLE_ICON /> },
  ...
  return (
    <Reveal>
      <div className="border border-rule bg-cream-warm p-8 md:p-10">
        ...
        <div className={`grid gap-px bg-rule ${slots.length === 4 ? "grid-cols-4" : slots.length === 3 ? "grid-cols-3" : slots.length === 2 ? "grid-cols-2" : "grid-cols-1"}`}>
          {slots.map((slot) => (

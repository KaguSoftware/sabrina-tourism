    ...
    <>
      ...
      <section className="relative z-5 min-h-[70vh] flex items-end pb-20 px-[clamp(20px,4vw,56px)] overflow-hidden text-cream">
        ...
        <div className="relative z-5 max-w-[1320px] mx-auto w-full">
          <Reveal>
            <Kicker className="kicker--dark">{t("kicker")}</Kicker>
          </Reveal>
          <Reveal delay={140}>
      ...
      {/* TABBED FORM */}
      <section className="relative z-10 px-[clamp(20px,4vw,56px)] py-[clamp(60px,8vw,100px)] overflow-hidden">
        ...
        <div className="max-w-[1320px] mx-auto relative z-7">
          <div className="relative inline-flex gap-4 border-b border-rule mb-12">
            {(["airport", "custom"] as const).map((val) => (
              <button
                key={val}
                role="tab"

...
function CompassIcon({ animate }: { animate: boolean }) {
  return (
    <svg viewBox="0 0 120 120" className="w-[100px] h-[100px]" aria-hidden="true">
      <circle cx="60" cy="60" r="48" fill="#efe4d2" stroke="#1b4d5c" strokeWidth="2.5" />
      <circle cx="60" cy="60" r="40" fill="none" stroke="#1b4d5c" strokeWidth="1" strokeDasharray="2 6" />
      <text x="60" y="22" textAnchor="middle" fontFamily="var(--font-fraunces),serif" fontSize="10" fill="#1b4d5c">N</text>
      <text x="60" y="107" textAnchor="middle" fontFamily="var(--font-fraunces),serif" fontSize="10" fill="#1b4d5c">S</text>
      <text x="13" y="64" textAnchor="middle" fontFamily="var(--font-fraunces),serif" fontSize="10" fill="#1b4d5c">W</text>
      <text x="107" y="64" textAnchor="middle" fontFamily="var(--font-fraunces),serif" fontSize="10" fill="#1b4d5c">E</text>
      <g
        style={{
          transformOrigin: "60px 60px",
          animation: animate ? "compass-spin 1400ms cubic-bezier(0.22,0.61,0.36,1) forwards" : undefined,
          transform: animate ? undefined : "rotate(140deg)",
        }}
      >
        <polygon points="60,28 64,60 60,92 56,60" fill="#c46b4f" stroke="#1b4d5c" strokeWidth="1.5" />
...
function SuitcaseIcon({ animate }: { animate: boolean }) {
  return (
    <svg viewBox="0 0 120 120" className="w-[100px] h-[100px]" aria-hidden="true">
      <ellipse cx="60" cy="107" rx="42" ry="4" fill="#1b4d5c"
        style={{ opacity: animate ? 0.35 : 0, transition: "opacity 800ms ease 200ms" }} />
      <rect x="20" y="48" width="80" height="56" rx="8" fill="#c99a3f" stroke="#1b4d5c" strokeWidth="2.5" />
      <rect x="36" y="60" width="48" height="32" rx="2" fill="#f5ede0" stroke="#1b4d5c" strokeWidth="1.5" />
      <line x1="36" y1="74" x2="84" y2="74" stroke="#1b4d5c" strokeWidth="1.5" strokeDasharray="3 3" />
      <g style={{
        transformOrigin: "60px 50px",
        transform: animate ? "rotate(-24deg) translate(-2px,-4px)" : "rotate(0deg)",
        transition: "transform 900ms cubic-bezier(0.34,1.56,0.64,1)",
      }}>
        <rect x="20" y="44" width="80" height="12" rx="4" fill="#c46b4f" stroke="#1b4d5c" strokeWidth="2.5" />
        <rect x="50" y="34" width="20" height="14" rx="3" fill="#c46b4f" stroke="#1b4d5c" strokeWidth="2.5" />
      </g>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 120 120" className="w-[100px] h-[100px]" aria-hidden="true">
      ...
      <path
        ...
        style={{ transition: animate ? "stroke-dashoffset 1100ms ease 200ms" : undefined }}
      />
      <path
        ...
        fill="#25d366"
      />
      <rect x="18" y="96" width="24" height="8" rx="2" fill="#c99a3f" opacity={animate ? 1 : 0}
        style={{ transition: "opacity 400ms ease 600ms" }} />
      <rect x="48" y="96" width="54" height="8" rx="2" fill="#c99a3f" opacity={animate ? 0.5 : 0}
...
function StepCard({ step, index }: { step: Step; index: number }) {
  ...
  return (
    <div ref={ref} className="pt-9 border-t border-rule relative">
      ...
      <span className="font-display italic text-ochre text-[24px] leading-none">{step.num}</span>
      <h3 className="font-display font-normal text-[clamp(22px,2.4vw,28px)] tracking-tight mt-3 mb-3.5">
        {step.heading}
      </h3>
      <p className="text-ink-soft text-[15px] leading-relaxed max-w-[38ch]">{step.body}</p>
    </div>
  );
}
